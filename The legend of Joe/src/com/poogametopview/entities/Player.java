...
public class Player extends Entity{
	...
	private boolean holdingPistol = false;
	...
	public Player(int x, int y, int wid, int hei, BufferedImage sprite) {
		...
		for(int i = 0; i < 4; i++) {
			rightPlayer[i] = Game.spritesheet.getSprite(16 * 0 + (i * 16), 16 * 7, 16, 16);
		}
		for(int i = 0; i < 4; i++) {
			leftPlayer[i] = Game.spritesheet.getSprite(16 * 0 + (i * 16), 16 * 8, 16, 16);
		}
	}
	...
	public void tick() {
		...
		if(shooting) {
			...
			if(holdingPistol && ammo > 0) {
				...
				ShotPistol pistolBullet = new ShotPistol(this.getX() + originX, this.getY() + originY, 3, 3, null, dirX, 0);
				Game.pistolBullets.add(pistolBullet);
			}
		}
	...
	public void checkPistolCollision() {
		...
		if(!isDamaged) {
			if(direction == rightDir) {
				...
				if(holdingPistol == true)
					gph.drawImage(Entity.PISTOL_RIGHT, this.getX() - Camera.x + 10, this.getY() - Camera.y + 1, null);
				...
				if(holdingPistol == true)
					gph.drawImage(Entity.PISTOL_LEFT, this.getX() - Camera.x - 11, this.getY() - Camera.y + 1, null);
