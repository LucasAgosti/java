...
public class Enemy2 extends Entity{
	...
	public Enemy2(int x, int y, int wid, int hei, BufferedImage sprite) {
		super(x, y, wid, hei, null);
		rightEnemy2 = new BufferedImage[2];
		leftEnemy2 = new BufferedImage[2];
		
		rightEnemy2[0] = Game.spritesheet.getSprite(16 * 7, 16 * 3, 16, 16);
		rightEnemy2[1] = Game.spritesheet.getSprite(16 * 8, 16 * 3, 16, 16);
		
		leftEnemy2[0] = Game.spritesheet.getSprite(16 * 7, 16 * 4, 16, 16);
		leftEnemy2[1] = Game.spritesheet.getSprite(16 * 8, 16 * 4, 16, 16);
	...
	public void render(Graphics gphs) {
		...
		if(!enDamaged) {
			if(direcao == 1) {
				gphs.drawImage(rightEnemy2[index], this.getX() - Camera.x, this.getY() - Camera.y, null);
			
			}else if(direcao == -1) {
				gphs.drawImage(leftEnemy2[index], this.getX() - Camera.x, this.getY() - Camera.y, null);
				
			}	
		}else if(enDamaged){
			gphs.drawImage(Entity.FEEDBACK2_EN, this.getX() - Camera.x, this.getY() - Camera.y, null);
		}
	...
	public boolean isColliding(int xNext, int yNext) {
		...
		for(int i = 0; i < Game.enemies2.size(); i++) {
			Enemy2 e2 = Game.enemies2.get(i);
			
			if(e2 == this)
				continue;
				
			Rectangle targetEnemy = new Rectangle(e2.getX() + maskX, e2.getY() + maskY, maskWid, maskHei);
